...
| 正式 APK | `release/penink-vpn.apk`（已簽名） |
...
> **架構**：Android App（Kotlin + Compose）透過 WebSocket(TLS) 隧道連回 Docker 容器內的 Node.js 伺服器（Colima Linux VM）。透過 Cloudflare Tunnel 反向代理。無需 Linux 實體主機。
